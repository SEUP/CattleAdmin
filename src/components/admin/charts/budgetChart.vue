  ...
  export default {
      ...
      methods : {
        ...
        load :async function () {
          ...
          this.chartData[0] = await  await this.$store.dispatch("admin/charts/getChart",QueryString)

          let QueryString2 ="range/farm-owner/total_budget/"+this.min + "/"+this.max + "/"+this.step ;
          if(this.province && this.province.province_id !=0){
            QueryString2 += "/" + this.province.province_id
          }
          this.chartData[1] = await this.$store.dispatch("admin/charts/getChart",QueryString2)
