...
<script>
  ...
  export default {
    ...
    async created () {
      ...
      this.items = await [defaultChoice].concat(await this.$store.dispatch("choices/getChoicesByType", 'use_land'));
      this.subItems = await this.$store.dispatch("choices/getChoicesByType", 'sub_use_lands');
