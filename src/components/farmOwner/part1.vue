    ...
    <div v-if="form">
      <v-layout column wrap>
        <v-flex xs12>
          ...
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <v-text-field label="ชื่อ" placeholder="กรุณากรอกชื่อ" v-model="form.first_name" @chage ="updateForm"></v-text-field>
            <v-text-field label="นามสกุล" placeholder="กรุณากรอกนามสกุล" v-model="form.last_name" @chage ="updateForm"></v-text-field>
            <v-text-field label="รหัสประจำตัวประชาชน" placeholder="กรุณากรอกรหัสประจำตัวประชาชน" v-model="form.person_id" @chage ="updateForm" ></v-text-field>
          </div>
        <v-card-text class="pa-2 title">1.2 ที่อยู่ตามสำเนาทะเบียนบ้าน</v-card-text>
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <v-text-field label="บ้านเลขที่" placeholder="บ้านเลขที่" v-model="form.house_no" @chage ="updateForm"></v-text-field>
            <v-text-field label="หมู่" placeholder="หมู่"  v-model="form.house_moo" @chage ="updateForm"></v-text-field>
            <district-select
            ...
            ></district-select>
            <v-text-field label="รหัสไปรษณีย์" placeholder="รหัสไปรษณีย์" v-model="form.house_postcode" @chage ="updateForm"></v-text-field>
            <v-text-field label="โทรศัพท์บ้าน" placeholder="โทรศัพท์บ้าน"  v-model="form.house_phone" @chage ="updateForm"></v-text-field>
            <v-text-field label="โทรศัพท์มือถือ" placeholder="โทรศัพท์มือถือ"  v-model="form.mobile_no" @chage ="updateForm"></v-text-field>
            <v-text-field label="Email" placeholder="Email"  v-model="form.email" @chage ="updateForm"></v-text-field>
          </div>
          <v-card-text class="pa-2 title">1.3 ที่อยู่ฟาร์ม</v-card-text>
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <v-text-field label="ที่ตั้งฟาร์มเลขที่" placeholder="ที่ตั้งฟาร์มเลขที่" v-model="form.farm_no" @chage ="updateForm"></v-text-field>
            <v-text-field label="หมู่" placeholder="หมู่" v-model="form.farm_moo" @chage ="updateForm"></v-text-field>
            <district-select
          ...
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <v-text-field label="พิกัดฟาร์ม (lat)" placeholder="พิกัดฟาร์ม (lat)" v-model="form.farm_lat" @chage ="updateForm"></v-text-field>
            <v-text-field label="พิกัดฟาร์ม (lng)" placeholder="พิกัดฟาร์ม (lng)" v-model="form.farm_long" @chage ="updateForm"></v-text-field>
          </div>
          ...
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <choice-select type="social_status" label="สถานภาพทางสังคม":value="form.social_status"
                           @change = "updateChoice(form.social_status)"></choice-select>
          </div>
          <v-card-text class="pa-2 title">1.8 การเลี้ยงโคเนื้อ</v-card-text>
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <choice-select type="cattle_job" label="การเลี้ยงโคเนื้อเป็น" :value="form.cattle_job"
                           @change = "updateChoice(form.cattle_job)"></choice-select>
          </div>
          <v-card-text class="pa-2 title">1.9 ท่านมีรายได้จากการประกอบอาชีพอะไรบ้าง (เลือกได้มากกว่า 1 ข้อ)</v-card-text>
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
              <choice-check-box type = "jobtypes" :value="form.jobtypes" @change = "updateJobType"></choice-check-box >
          </div>
          <v-card-text >1.10 ท่านมีรายได้รวมของครัวเรือนเฉลี่ยเท่าไหร่ (บาท/ปี)</v-card-text>
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <choice-select type="income_range" label="ท่านมีรายได้รวมของครัวเรือนเฉลี่ยเท่าไหร่ (บาท/ปี)" :value="form.income_range"
                           @change = "updateChoice" ></choice-select>
          </div>
          <v-card-text class="pa-2 title">1.11 รายได้เฉลี่ยต่อปีของการขายโคเนื้อ (บาท)</v-card-text>
          <v-divider></v-divider>
          <div class="ma-2 mx-4">
            <v-text-field placeholder="รายได้เฉลี่ยต่อปีของการขายโคเนื้อ (บาท)" label="รายได้เฉลี่ยต่อปีของการขายโคเนื้อ (บาท)"
                          hint="*หากยังไม่มีรายได้จากการขายโคเนื้อ ให้เว้นว่างช่องนี้ไว้" persistent-hint type="number"
                          v-model="form.avg_cattle_income" @chage ="updateForm"
            ></v-text-field>
          </div>
...
<script>
    ...
    export default {
      ...
      methods : {
        ...
        updateChoice :async function (value,type) {
          // await console.log("B",type)
          // type =await value
